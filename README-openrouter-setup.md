...
timeout של 15 שניות לכל בקשה בודדת (`PER_CALL_TIMEOUT_MS`) — כדי שאם
...
for response"). הפונקציה גם מבקשת מ-Vercel `maxDuration: 30` שניות
(נתמך בתוכנית ה-Hobby החינמית עד 60 שניות) — שילוב שאמור לתת מרווח
בטוח מעל ה-15 שניות של כל ניסיון בודד.
